 ...
 */
async function getCityCode(address: string, key: string): Promise<string | null> {
  try {
 ...
 */
async function getWeatherInfo(cityCode: string, key: string, extensions: string = "base"): Promise<any> {
  try {
 ...
 */
function formatLiveWeather(weatherData: any): string {
 ...
 * @returns string 格式化后的天气信息
 */
function formatForecastWeather(weatherData: any): string {
  ...
  if (forecast.casts && forecast.casts.length > 0) {
    ...
    forecast.casts.forEach((cast: any, index: number) => {
 ...
 */
function getCurrentDateString(timezone?: string): string {
  const now = new Date();
